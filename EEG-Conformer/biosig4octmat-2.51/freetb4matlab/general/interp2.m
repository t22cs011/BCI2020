...
function ZI = interp2 (varargin)
  ...
  if (strcmp (method, 'linear') || strcmp (method, 'nearest') ...
    ...
    if (size(Z,2) ~= length (X) || size(Z,1) ~= length (Y))
    ...
    if (strcmp (method, 'linear'))
      ...
      o2m_tmp_1 = diff (X);
Xsc = (XI - X(xidx)) ./ (o2m_tmp_1(xidx));
      o2m_tmp_2 = diff (Y);
Ysc = (YI - Y(yidx)) ./ (o2m_tmp_2(yidx));
    ...
    elseif (strcmp (method, 'pchip'))
      ...
      DX = o2m_tmp___pchip_deriv__ (X, Z, 2);
      DY = o2m_tmp___pchip_deriv__ (Y, Z, 1);
      %# Compute mixed derivatives row-wise and column-wise, use the average.
      DXY = (__pchip_deriv__ (X, DY, 2) + o2m_tmp___pchip_deriv__ (Y, DX, 1))/2;
      ...
      for i = 1:2
        for j = 1:2
          ...
          ZI = ZI + (xb{1,i} .* yb{1,j} .*   Z(zidx));
          ZI = ZI + (xb{2,i} .* yb{1,j} .*  DX(zidx));
          ZI = ZI + (xb{1,i} .* yb{2,j} .*  DY(zidx));
          ZI = ZI + (xb{2,i} .* yb{2,j} .* DXY(zidx));
        end
      end
    ...
    if (strcmp (method, 'cubic'))
      ...
      elseif (isgriddata (X) && isgriddata (Y'))
        ...
        XI = (size(Z,2) - 1) * (XI - X (1)) / (X (end) - X (1)) + 1;
        YI = (size(Z,1) - 1) * (YI - Y (1)) / (Y (end) - Y (1)) + 1;
    ...
    elseif (strcmp (method, 'spline'))
      if (isgriddata (XI) && isgriddata (YI'))
        ZI = o2m_tmp___splinen__ ({Y(:,1).', X(1,:)}, Z, {YI(:,1), XI(1,:)}, extrapval, 
                        'spline');
      else
        error ('interp2: input data must have `meshgrid'' format');
      end
