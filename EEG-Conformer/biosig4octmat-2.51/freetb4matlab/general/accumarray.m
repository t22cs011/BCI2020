...
function A = accumarray (subs, val, sz = [], func = [], fillval = [], isspar = [])  
  ...
  if (isspar)
    ...
    if (~ (isempty (func) || func == @sum))
      ...
      o2m_tmp_1 = val(:);
val = cellfun (func, mat2cell (o2m_tmp_1(idx), diff ([0; jdx])));
      subs = subs(jdx, :);
      mode = 'unique';
    else
      mode = 'sum';
    ...
    if (isempty (func) || func == @sum)
      %# Fast summation.
      if (isempty (sz))
        A = o2m_tmp___accumarray_sum__ (subs, val);
      else
        A = o2m_tmp___accumarray_sum__ (subs, val, prod (sz));
        %# set proper shape.
        A = reshape (A, sz);
      end
    ...
    elseif (func == @max)
      ...
      if (isempty (sz))
        A = o2m_tmp___accumarray_max__ (subs, val, zero);
      else
        A = o2m_tmp___accumarray_max__ (subs, val, zero, prod (sz));
        A = reshape (A, sz);
      end
    ...
    elseif (func == @min)
      ...
      if (isempty (sz))
        A = o2m_tmp___accumarray_min__ (subs, val, zero);
      else
        A = o2m_tmp___accumarray_min__ (subs, val, zero, prod (sz));
        A = reshape (A, sz);
      end
      ...
      o2m_tmp_2 = (x);
persistent simple_cell_str = func2str (@o2m_tmp_2 {x});
