  ...
  if (b < a)
      ...
      if (~ ischar (varargin{1}))
        ...
        while (idx < nargin - 3)
          if (ischar (varargin{idx}))
            str = varargin{idx}; idx = idx + 1;
            if (strcmpi (str, 'reltol'))
              reltol = varargin{idx}; idx = idx + 1;
            elseif (strcmpi (str, 'abstol'))
              abstol = varargin{idx}; idx = idx + 1;
            elseif (strcmpi (str, 'waypoints'))
              waypoints = reshape(varargin{idx},[],1);
              idx = idx + 1;
              if (isreal(waypoints))
                waypoints (waypoints < a | waypoints > b) = [];
              end
            elseif (strcmpi (str, 'maxintervalcount'))
              maxint = varargin{idx}; idx = idx + 1;
            elseif (strcmpi (str, 'trace'))
              trace = varargin{idx}; idx = idx + 1;
            else
    ...
    elseif (isinf (a) && isinf(b))
      ...
      if (~isempty (waypoints))
        o2m_tmp_1 = (x);
trans = @o2m_tmp_1 (2 * x) ./ (1 + sqrt(1 + 4 * x .^ 2));
    ...
    elseif (isinf(b))
      ...
      if (~ isempty (waypoints))
        trans = @o2m_tmp___quadgk_finite_waypoint__;
      ...
      h0 = b - a;
      o2m_tmp_2 = (t);
trans = @o2m_tmp_2 ((b - a) ./ 4) * t .* (3 - t.^2) + (b + a) ./ 2;
    ...
    try
      ...
      [q_subs, q_errs] = o2m_tmp___quadgk_eval__ (f, subs);
      q0 = sum (q_subs);
      err0 = sum (q_errs);
...
function [q, err] = o2m_tmp___quadgk_eval__ (f, subs)
...
function t = o2m_tmp___quadgk_finite_waypoint__ (x, a, b)
  c = (-4 .* x + 2.* (b + a)) ./ (b - a);
  k = ((sqrt(c .^ 2 - 4) + c) ./ 2) .^ (1/3);
