...
function manova (Y, g)
  ...
  if (~isvector (g) || (length (g) ~= n))
    error ('manova: g must be a vector of length size(Y,1)');
  end
  ...
  if (0)
    ...
    fprintf(1,'\n');
    fprintf(1,'One-way MANOVA Table:\n');
    fprintf(1,'\n');
    fprintf(1,'Test             Test Statistic      Approximate p\n');
    fprintf(1,'^^^^^^^^^^^^^^^^^^^^^^^^^\n');
    fprintf(1,'Wilks            %10.4f           %10.9f \n', Lambda, W_pval_1);
    fprintf(1,'                                      %10.9f \n', W_pval_2);
    fprintf(1,'Hotelling-Lawley %10.4f           %10.9f \n', HL, HL_pval);
    fprintf(1,'Pillai-Bartlett  %10.4f           %10.9f \n', PB, PB_pval);
    fprintf(1,'\n');

  end

  fprintf(1,'\n');
  fprintf(1,'MANOVA Results:\n');
  fprintf(1,'\n');
  fprintf(1,'# of groups:    %d\n', k);
  fprintf(1,'# of samples:   %d\n', n);
  fprintf(1,'# of variables: %d\n', p);
  fprintf(1,'\n');
  fprintf(1,'Wilks'' Lambda:  %5.4f\n', Lambda);
  fprintf(1,'Approximate p:  %10.9f (chisquare approximation)\n', W_pval_1);
  fprintf(1,'                 %10.9f (F approximation)\n', W_pval_2);
  fprintf(1,'\n');
